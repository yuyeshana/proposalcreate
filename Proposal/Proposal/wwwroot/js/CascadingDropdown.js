$(document).ready(function () {
    ...
    // 第一次審査者の初期値を設定
    setupFirstReviewerInitialValues();
...
// 第一次審査者の初期値を設定する関数
function setupFirstReviewerInitialValues() {
    // 所属に値がある場合、部・署を読み込む
    var affiliationId = $('#firstReviewerAffiliation').val();
    if (affiliationId) {
        loadDepartments(affiliationId);

        // 部・署に値がある場合、課・部門を読み込む
        setTimeout(function () {
            var departmentId = $('#firstReviewerDepartment').val();
            if (departmentId) {
                loadSections(departmentId);

                // 課・部門に値がある場合、係・担当を読み込む
                setTimeout(function () {
                    var sectionId = $('#firstReviewerSection').val();
                    if (sectionId) {
                        loadSubsections(sectionId);
                    }
                }, 100);
